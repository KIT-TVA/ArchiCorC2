...
public class CompatibilityChecker {
	public static boolean horizontal(Interface provided, Interface required) {
		for(Method m : required.getMethods()) {
			if(!provided.getMethods().stream().map(elem -> elem.getName()).collect(Collectors.toList()).contains(m.getName())) {
				return false; // Required method not found
			}
			// Get matching method by name
			Method match = provided.getMethods().stream().filter(elem -> elem.getName().equals(m.getName())).findFirst().get();
			// Check signature
	...
	public static boolean checkSignature(Method m1, Method m2)  { // very simple check; no covariant/contravariant etc... signatures have to match exactly!
	...
	public static boolean checkLiskov(Method m1, Method m2)  { // very simple check; no covariant/contravariant etc... signatures have to match exactly!
		//
...
//		return false;
		...
		List<String> parameters = m1.getInArgs().stream().map(arg -> DummyJavaFileWriter.getType(arg.getType())).collect(Collectors.toList());
		int[] res = KeYCompatibilityProver.proveWithKeYByText(m1.getName() + "_m1", parameters.toArray(new String[0]), content);
		switch(res[0]) {
		case 0: // Proven
			System.out.println(m1.getName() + " is correctly matched.");
			return true;
		case 1: 
			System.out.println("Matching of " + m1.getName() + " and  " +  m2.getName() + " is interactively proven.");
			return true;
		case 2: 
			System.out.println("Matching of " + m1.getName() + " and  " +  m2.getName() + " leads to a timeout.");
			return false;
		case 3: 
			System.out.println("Matching of " + m1.getName() + " and  " +  m2.getName() + " leads to open goals.");
			return false;
		case 4: 
			System.out.println("Matching of " + m1.getName() + " and  " +  m2.getName() + " not provable (check syntax?).");
			return false;
		case 5: 
			System.out.println("Matching of " + m1.getName() + " and  " +  m2.getName() + " not provable due to unknown reasons.");
			return false;
		}
